...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 reweight_func=None,  # None, 'inv', 'sqrt_inv', 'rebalance', 'CB'  对label的重新设置权重过程
    ...
    def CB_weight(self, gt_labels):
        if  'by_class' in self.CB_mode:
            weight = torch.tensor((1 - self.CB_beta)).cpu() / \
                     (1 - torch.pow(self.CB_beta, self.class_freq)).cpu()
        elif 'average_n' in self.CB_mode:
            avg_n = torch.sum(gt_labels * self.class_freq, dim=1, keepdim=True) / \
                    torch.sum(gt_labels, dim=1, keepdim=True)
            weight = torch.tensor((1 - self.CB_beta)).cpu() / \
                     (1 - torch.pow(self.CB_beta, avg_n)).cpu()
        elif 'average_w' in self.CB_mode:
            weight_ = torch.tensor((1 - self.CB_beta)).cpu() / \
                      (1 - torch.pow(self.CB_beta, self.class_freq)).cpu()
            ...
            weight = torch.tensor((1 - self.CB_beta)).cpu() / \
                     (1 - torch.pow(self.CB_beta, min_n)).cpu()
        else:
            raise NameError
        return weight
...
def get_loss_func(loss_func_name, class_freq, train_num, alpha=0.5, gamma=4):
    if loss_func_name == 'BCE':
        loss_func = ResampleLoss(reweight_func=None, loss_weight=1.0,
                                focal=dict(focal=False, alpha=alpha, gamma=gamma),
                                logit_reg=dict(),
                                class_freq=class_freq, train_num=train_num)
        
    if loss_func_name == 'FL':
        loss_func = ResampleLoss(reweight_func=None, loss_weight=1.0,
                                focal=dict(focal=True, alpha=alpha, gamma=gamma),
                                logit_reg=dict(),
                                class_freq=class_freq, train_num=train_num) 
    ...
    if loss_func_name == 'CBloss': #CB
        loss_func1 = ResampleLoss(reweight_func='CB', loss_weight=5.0,
                                focal=dict(focal=True, alpha=0.95, gamma=4),
                                logit_reg=dict(),
    ...
    if loss_func_name == 'DBloss-noFocal': # DB-0FL
        loss_func = ResampleLoss(reweight_func='rebalance', loss_weight=0.5,
                                focal=dict(focal=False, alpha=0.95, gamma=4),
                                logit_reg=dict(init_bias=0.05, neg_scale=2.0),
                                map_param=dict(alpha=0.1, beta=10.0, gamma=0.05), 
                                class_freq=class_freq, train_num=train_num)
